    ...
    def Bilateral(self,eps_prices,MAXIT,lbda,delta_tol,inspection):
        ...
        for K in range(MAXIT):
            ...
            if np.max(np.std(self.pag,axis=0)) < eps_prices:
                ...
                elif inspection[0] =='fixed':
                    agents_insp1 = inspection[1]
                    agents_insp2 = inspection[1]
                    goods_insp = inspection[2]
                else:
...
#            model.e = Param(model.n,initialize=e_init)
            ...
            model.x = Var(model.n, within=NonNegativeReals,initialize=x_init)
            def obj_rule(model):
#                return sum( model.alpha[j]*log(model.x[j]) for j in model.n )
                return prod( (model.x[j])**model.alpha[j] for j in model.n )
            model.obj = Objective(rule=obj_rule, sense=maximize)
            def bc_rule(model):
                return sum( model.p[j]*(model.x[j]-self.e[ii,j]) for j in model.n) <= 0
        ...
        for i in range(self.I):
            for j in range(self.n):
                if j == 0:
                    ...
                    xx[i,j] = self.alpha[i,j]*xx[i,0]/p[j]
        return p, xx
